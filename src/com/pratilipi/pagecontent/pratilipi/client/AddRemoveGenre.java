...
public class AddRemoveGenre extends Composite implements HasValueChangeHandlers<PratilipiData> {
	...
	public AddRemoveGenre() {
		...
		pratilipiService.getGenreList( new GetGenreListRequest(),
			...
			public void onSuccess( GetGenreListResponse response ) {
				genreDataList = response.getGenreDataList();
				setGenreData( genreDataList, pratilipiData );
			}
	...
	private void setGenreData( List<GenreData> genreDataList, final PratilipiData pratilipiData ) {
		
		if( genreDataList == null || pratilipiData == null )
			return;
